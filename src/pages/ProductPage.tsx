...
import { useStore } from '../contexts/StoreContext';
  ...
  useEffect(() => {
    // Buscar produtos do contexto global window (simula contexto global para página única)
    // @ts-ignore
    const globalProducts = window.__GLOBAL_PRODUCTS__ || [];
  ...
  useEffect(() => {
    if (products && id) {
      ...
      setReviews([
        { id: 1, customer_name: 'Cliente 1', rating: 5, comment: 'Ótimo produto!', created_at: new Date().toISOString() },
        { id: 2, customer_name: 'Cliente 2', rating: 4, comment: 'Gostei bastante.', created_at: new Date().toISOString() },
      ]);
    }
  }, [products, id]);

  if (!product) {
    return <div className="min-h-screen flex items-center justify-center text-xl">Produto não encontrado</div>;
  }
